  ...
  def cache_words
    ...
    [Language::POLISH_ID, Language::ENGLISH_ID].each do |lang_id|
      Word.with_definitions(lang_id).each do |word|
        counter += 2
        Redis.new.mset(word_key(word.value_slug, Language::ENGLISH_ID, lang_id), serialize(word))
        Redis.new.mset(word_key(word.id, Language::ENGLISH_ID, lang_id), serialize(word))
      end
    end
